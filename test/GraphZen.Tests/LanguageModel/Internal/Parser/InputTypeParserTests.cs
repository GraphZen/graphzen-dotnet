    ...
    public class InputTypeParserTests : ParserTestBase
    {
        ...
        public void ParseNonNullableListType()
        {
            ParseType("[Foo]!").Should()
                .Be(SyntaxFactory.NonNullType(
                    SyntaxFactory.ListType(SyntaxFactory.NamedType(SyntaxFactory.Name("Foo")))));
        }
        ...
        {
            ParseType("Foo!").Should()
                .Be(SyntaxFactory.NonNullType(SyntaxFactory.NamedType(SyntaxFactory.Name("Foo"))));
        }
    }
}