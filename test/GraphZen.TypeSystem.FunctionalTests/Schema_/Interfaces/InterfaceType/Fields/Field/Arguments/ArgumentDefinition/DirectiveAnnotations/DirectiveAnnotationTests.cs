    ...
    DirectiveAnnotations
{
    ...
    public class DirectiveAnnotationTests
    {
                ...
                _.Interface("Foo").Field("Bar", "String", f =>
                {
                    f.Argument("arg", "String", a =>
                    {
                        ...
                        add.Should().Throw<InvalidOperationException>().WithMessage(
                            "Cannot annotate interface field argument Foo.Bar.arg with directive bar: Directive bar has not been defined yet.");
                    });
                });
            });
        }
        ...
        public void directive_annotations_can_be_removed_()
        {
            ...
            schema.GetInterface("Baz").GetField("Bar").GetArgument("arg").DirectiveAnnotations.Should().BeEmpty();
        }
