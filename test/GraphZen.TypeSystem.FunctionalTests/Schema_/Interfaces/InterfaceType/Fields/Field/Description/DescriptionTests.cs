...
namespace GraphZen.TypeSystem.FunctionalTests.Schema_.Interfaces.InterfaceType.Fields.Field.Description
{
    ...
    public class DescriptionTests
    {
        ...
        {
            var schema =
                Schema.Create(_ => { _.Interface("foo").Field("foo", "String", f => f.Description("desc")); });
            schema.GetInterface("foo").GetField("foo").Description.Should().Be("desc");
        }
        ...
        public void description_can_be_removed_()
        {
            ...
            {
                _.Interface("foo").Field("foo", "String", f => { f.Description("desc").RemoveDescription(); });
            });
            schema.GetInterface("foo").GetField("foo").Description.Should().BeNull();
        }
    }
}