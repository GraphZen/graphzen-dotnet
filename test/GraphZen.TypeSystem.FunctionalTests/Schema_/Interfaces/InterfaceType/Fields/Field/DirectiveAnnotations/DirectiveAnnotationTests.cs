    ...
    public class DirectiveAnnotationTests
    {
        ...
        public void directive_annotation_can_be_added_()
        {
            var schema = Schema.Create(_ =>
            {
                ...
                _.Interface("Foo").Field("Bar", "String", f => { f.AddDirectiveAnnotation("Foo", "test"); });
            });
            schema.GetInterface("Foo").GetField("Bar").FindDirectiveAnnotations("Foo").Single().Value.Should()
                .Be("test");
        }
            ...
            Schema.Create(_ =>
            {
                _.Interface("Foo").Field("Bar", "String", f =>
                {
                    ...
                    add.Should().Throw<InvalidOperationException>().WithMessage(
                        "Cannot annotate interface field Foo.Bar with directive bar: Directive bar has not been defined yet.");
                });
            });
        }
        ...
        public void directive_annotation_cannot_be_added_unless_location_is_valid_()
        {
            Schema.Create(_ =>
            {
                ...
                _.Interface("Foo").Field("Bar", "String", f =>
                {
                    Action add = () => f.AddDirectiveAnnotation("bar", "test");
                    add.Should().Throw<InvalidOperationException>().WithMessage(
                        "Cannot annotate interface field Foo.Bar with directive bar: Directive bar cannot be annotated on field definitions because it is only valid on queries, the schema, or arguments.");
                });
            });
        }
                ...
                _.Interface("Foo").Field("Bar", "String", f =>
                {
                    ...
                    adds.ForEach(add =>
                    {
                        add.Should().Throw<InvalidNameException>().WithMessage(
                            $"Cannot annotate interface field Foo.Bar with directive: \"{name}\" is not a valid directive name.");
                    });
                });
            });
        }
        ...
        public void directive_annotations_can_be_removed_()
        {
            ...
            schema.GetInterface("Baz").GetField("Bar").DirectiveAnnotations.Should().BeEmpty();
        }
        ...
        public void directive_annotations_can_be_removed_by_name_()
        {
            ...
            var bar = schema.GetInterface("Baz").GetField("Bar");
        ...
        public void directive_annotations_are_removed_when_directive_is_removed_()
        {
            ...
            {
                _.Directive("foo").Locations(DirectiveLocation.FieldDefinition);
                _.Interface("Foo").Field("Baz", "String", f => { f.AddDirectiveAnnotation("foo"); });
                _.RemoveDirective("foo");
            });
            var baz = schema.GetInterface("Foo").GetField("Baz");
        ...
        public void directive_annotations_are_renamed_when_directive_is_renamed_()
        {
            ...
            {
                _.Directive("foo").Locations(DirectiveLocation.FieldDefinition);
                _.Interface("Baz").Field("Bar", "String", f => { f.AddDirectiveAnnotation("foo", "test"); });
