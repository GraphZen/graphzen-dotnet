...
namespace GraphZen.TypeSystem.FunctionalTests.Schema_.Interfaces.InterfaceType.DirectiveAnnotations
{
    ...
    public class DirectiveAnnotationTests
    {
        ...
        public void directive_annotation_can_be_added_()
        {
            ...
            {
                _.Directive("Foo").Locations(DirectiveLocation.Interface);
                _.Interface("Foo").AddDirectiveAnnotation("Foo", "test");
            });
            schema.GetInterface("Foo").FindDirectiveAnnotations("Foo").Single().Value.Should().Be("test");
        }
        ...
        public void directive_annotation_cannot_be_added_unless_directive_is_defined_()
        {
            Schema.Create(_ =>
            {
                ...
                add.Should().Throw<InvalidOperationException>().WithMessage(
                    "Cannot annotate interface Foo with directive bar: Directive bar has not been defined yet.");
            });
        }
        ...
        public void directive_annotation_cannot_be_added_unless_location_is_valid_()
        {
            Schema.Create(_ =>
            {
                ...
                Action add = () => foo.AddDirectiveAnnotation("bar", "test");
                add.Should().Throw<InvalidOperationException>().WithMessage(
                    "Cannot annotate interface Foo with directive bar: Directive bar cannot be annotated on interfaces because it is only valid on queries, the schema, or arguments.");
            });
        }
            ...
            Schema.Create(_ =>
            {
                ...
                adds.ForEach(add =>
                {
                    add.Should().Throw<InvalidNameException>().WithMessage(
                        $"Cannot annotate interface Foo with directive: \"{name}\" is not a valid directive name.");
                });
            });
        }
        ...
        public void directive_annotations_can_be_removed_()
        {
            ...
            {
                _.Directive("foo").Locations(DirectiveLocation.Interface);
                _.Directive("bar").Locations(DirectiveLocation.Interface);
                _.Interface("Baz")
        ...
        public void directive_annotations_can_be_removed_by_name_()
        {
            ...
            var baz = schema.GetInterface("Baz");
