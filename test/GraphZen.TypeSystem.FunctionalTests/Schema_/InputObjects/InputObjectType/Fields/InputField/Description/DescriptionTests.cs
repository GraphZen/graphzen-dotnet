...
namespace GraphZen.TypeSystem.FunctionalTests.Schema_.InputObjects.InputObjectType.Fields.InputField.Description
{
    ...
    public class DescriptionTests
    {
        ...
        {
            var schema =
                Schema.Create(_ => { _.InputObject("foo").Field("foo", "String", f => f.Description("desc")); });
            schema.GetInputObject("foo").GetField("foo").Description.Should().Be("desc");
        }
        ...
        public void description_cannot_be_null_()
        {
            Schema.Create(_ =>
            {
                _.InputObject("foo").Field("foo", "String", f =>
                {
        ...
        public void description_can_be_removed_()
        {
            ...
            {
                _.InputObject("foo").Field("foo", "String", f => { f.Description("desc").RemoveDescription(); });
            });
            schema.GetInputObject("foo").GetField("foo").Description.Should().BeNull();
        }
    }
}