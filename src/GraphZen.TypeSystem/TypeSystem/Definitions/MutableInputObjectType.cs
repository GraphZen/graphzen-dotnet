...
namespace GraphZen.TypeSystem
{
    ...
    public partial class MutableInputObjectType : MutableNamedTypeDefinition, IMutableInputObjectType
    {
        private readonly Dictionary<string, MutableInputField> _fields =
            new Dictionary<string, MutableInputField>();

        private readonly Dictionary<string, ConfigurationSource> _ignoredFields =
            new Dictionary<string, ConfigurationSource>();

        public MutableInputObjectType(TypeIdentity identity, MutableSchema schema,
        ...
        [GenDictionaryAccessors(nameof(MutableInputField.Name), "Field")]
        public IReadOnlyDictionary<string, MutableInputField> FieldMap => _fields;

        public IReadOnlyCollection<MutableInputField> Fields => _fields.Values;
        ...
        public bool RenameField(MutableInputField field, string name,
            ConfigurationSource configurationSource)
        {
            ...
            if (TryGetField(name, out var existing) && !existing.Equals(field))
            {
                throw new DuplicateItemException(
                    TypeSystemExceptions.DuplicateItemException.CannotRenameInputField(field, name));
            }
        ...
        public MutableInputField? FindField(MemberInfo member)
        {
        ...
        private bool IgnoreField(MutableInputField field, ConfigurationSource configurationSource)
        {
        ...
        public bool AddField(MutableInputField field)
        {
        ...
        public MutableInputField AddField(PropertyInfo property,
            ConfigurationSource configurationSource)
        {
            if (ClrType == null)
            {
            ...
            var field = new MutableInputField(fieldName, fieldTypeId, typeNode, nameConfigurationSource, Schema,
                configurationSource,
                property, this);
        ...
        public bool RemoveField(MutableInputField field)
        {
            _fields.Remove(field.Name);
            return true;
        }
        ...
        public MutableInputField? GetOrAddField(string name, Type clrType, ConfigurationSource configurationSource)
        {
            ...
            field = new MutableInputField(name, fieldTypeIdentity, typeSyntax, configurationSource, Schema,
                configurationSource, null, this);
            AddField(field);
            return field;
        }

        public MutableInputField? GetOrAddField(string name, string type, ConfigurationSource configurationSource)
        {
            var ignoredConfigurationSource = FindIgnoredFieldConfigurationSource(name);
            if (ignoredConfigurationSource.HasValue)
            {
            ...
            field = new MutableInputField(name, fieldTypeIdentity,
        ...
        IReadOnlyCollection<IInputField> IInputFields.Fields => Fields;
    }
}