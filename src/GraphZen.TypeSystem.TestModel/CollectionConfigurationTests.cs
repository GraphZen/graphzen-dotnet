...
namespace GraphZen
{
    public class CollectionConfigurationTests
    {
        ...
        private void ExecuteFixtures(Action<ICollectionElementConfigurationFixture> fixtureAction)
        {
        ...
        public void when_item_added_explicitly_item_configurationSource_should_be_explicit()
        {
            foreach (var fixture in GetFixtures())
            {
                ...
                var schema = Schema.Create(sb =>
                {
                    ...
                    (defCollection[itemName] as MemberDefinition).GetConfigurationSource().Should().Be(ConfigurationSource.Explicit);
